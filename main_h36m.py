from common.arguments_h36m import parse_args
import os
args = parse_args()
...
from common.D3DP_Grap2Eq import *
...
if args.keypoints == 'detectron_pt_coco':
    keypoints = coco2human36m(keypoints)
...
model_pos_train = D3DP_Grap2Eq(args, joints_left, joints_right, num_joints, is_train=True, spatial_adj = spatial_adj, temporal_adj = temporal_adj)
model_pos_test_temp = D3DP_Grap2Eq(args,joints_left, joints_right, num_joints, is_train=False, spatial_adj = spatial_adj, temporal_adj = temporal_adj)
model_pos = D3DP_Grap2Eq(args,joints_left, joints_right, num_joints, is_train=False, num_proposals=args.num_proposals, sampling_timesteps=args.sampling_timesteps, spatial_adj = spatial_adj, temporal_adj = temporal_adj)
...
#random_2d = torch.randint(low=0, high=10, size=(args.batch_size, args.number_of_frames, num_joints, 2), dtype=torch.long).cuda()
#random_3d = torch.randint(low=0, high=10, size=(args.batch_size, args.number_of_frames, num_joints, 3), dtype=torch.long).cuda()
...
#random_x = (random_2d, random_3d, random_input_text, random_pretext)
...
if not args.evaluate:
    ...
    while epoch < args.epochs:
        ...
        if epoch % args.checkpoint_frequency == 0 and epoch >= 20:
...
def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False, newmodel=None):
    ...
    with torch.no_grad():
        if newmodel is not None:
            ...
            if not use_trajectory_model:
                ...
                if args.evaluate == '':
                    chk_file_path = os.path.join(args.checkpoint, 'best_epoch_20_10.bin')
    ...
    if args.p2:
        return FPS, e1, e1_h, e1_mean, e1_select, e2, e2_h, e2_mean, e2_select
    else:
        return FPS, e1, e1_h, e1_mean, e1_select
...
if args.render:
    ...
    def run_evaluation(actions, action_filter=None):
        ...
        for action_key in actions.keys():
            ...
            if args.p2:
                FPS, e1, e1_h, e1_mean, e1_select, e2, e2_h, e2_mean, e2_select = evaluate(gen, action_key)
            else:
                FPS, e1, e1_h, e1_mean, e1_select = evaluate(gen, action_key)
