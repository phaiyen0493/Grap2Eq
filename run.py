...
if args.dataset == 'h36m':
    ...
    if args.keypoints == 'detectron_pt_coco':
        keypoints = coco2humaneva(keypoints)
    connections = {0: [11,8], 1: [2,5,14], 2: [1,3], 3: [2,4], 4: [3], 5: [1,6], 6: [5,7], 7: [6], 8: [0,9], 9: [8,10], 10: [9], 11: [0,12], 12: [11,13], 13: [12], 14: [1] }
...
spatial_adj = init_spatial_adj(connections, num_joints)
temporal_adj = init_temporal_adj(receptive_field, args.temporal_alpha, args.temporal_beta)

if args.lite_ver == False:
    model_pos_train =  Grap2Eq(num_frame= args.number_of_frames, num_joints=num_joints, in_chans=2, embed_dim_ratio=args.cs, depth=args.dep,
        num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,drop_path_rate=0.1, spatial_adj = spatial_adj, temporal_adj = temporal_adj, is_train = True)

    model_pos =  Grap2Eq(num_frame= args.number_of_frames, num_joints=num_joints, in_chans=2, embed_dim_ratio=args.cs, depth=args.dep,
        num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,drop_path_rate=0, spatial_adj = spatial_adj, temporal_adj = temporal_adj, is_train = False)        
else:
    model_pos_train =  Grap2Eq_Lite(num_frame= args.number_of_frames, num_joints=num_joints, in_chans=2, embed_dim_ratio=args.cs, depth=args.dep,
        num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,drop_path_rate=0.1, spatial_adj = spatial_adj, temporal_adj = temporal_adj, is_train = True)

    model_pos =  Grap2Eq_Lite(num_frame= args.number_of_frames, num_joints=num_joints, in_chans=2, embed_dim_ratio=args.cs, depth=args.dep,
        num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,drop_path_rate=0, spatial_adj = spatial_adj, temporal_adj = temporal_adj, is_train = False)        
...
random_2d = torch.randint(low=0, high=10, size=(args.batch_size, args.number_of_frames, num_joints, 2), dtype=torch.float).cuda()
random_input_text = torch.randint(low=0, high=1, size=(4,77), dtype=torch.int).cuda()
random_pretext = torch.randint(low=0, high=1, size=(4, 6,77), dtype=torch.int).cuda()
random_x = (random_2d, random_input_text, random_pretext)
train_macs = profile_macs(model_pos_train.cuda(), random_x)
...
print('INFO: Train MACS: ', train_macs/1000000000000, 'G')
...
if not args.evaluate:
    ...
    train_generator = ChunkedGenerator_Seq(args.batch_size, cameras_train, poses_train, poses_train_2d, action_train, args.number_of_frames,args.stride,
    ...
    while epoch < args.epochs:
        ...
        for cameras_train, batch_3d, batch_2d, batch_act in train_generator.next_epoch():
            ...
            # Predict 3D poses
            predicted_3d_pos = model_pos_train(inputs_2d, input_text, pre_text_tensor_train)
        ...
        with torch.no_grad():
            ...
            if not args.no_eval:
                ...
                for cam, batch, batch_2d, batch_act in test_generator.next_epoch():
                    ...
                    predicted_3d_pos = model_pos(inputs_2d, input_text, pre_text_tensor_valid)
                    predicted_3d_pos_flip = model_pos(inputs_2d_flip, input_text, pre_text_tensor_valid)
                ...
                N = 0
                for cam, batch, batch_2d, batch_act in train_generator_eval.next_epoch():
                    for i in range(batch_act.shape[0]):
                        batch_act[i] = batch_act[i].split(" ")[0]
                    input_text = []
                    for i in batch_act:
                    ...
                    pre_text_tensor_train_valid = pre_text_tensor.unsqueeze(dim=0)
                    pre_text_tensor_train_valid = pre_text_tensor_train_valid.repeat(input_text.shape[0], 1, 1)

                    if torch.cuda.is_available():
                        inputs_3d = inputs_3d.cuda()
                        inputs_2d = inputs_2d.cuda()
                        input_text = input_text.cuda()
                        pre_text_tensor_train_valid = pre_text_tensor_train_valid.cuda()

                    inputs_3d[:, :, 0] = 0

                    # Compute 3D poses
                    predicted_3d_pos = model_pos(inputs_2d, input_text, pre_text_tensor_train_valid)
...
def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False, newmodel=None):
    ...
    with torch.no_grad():
        ...
        N = 0
        FPS = 0
        for _, batch, batch_2d, batch_act in test_generator.next_epoch():
            inputs_2d = torch.from_numpy(batch_2d.astype('float32'))
            inputs_3d = torch.from_numpy(batch.astype('float32'))
            ...
            # Measure the FPS
            start_time1 = time()
            predicted_3d_pos = model_eval(inputs_2d, input_text, pre_text_tensor_valid)
            predicted_3d_pos_flip = model_eval(inputs_2d_flip, input_text, pre_text_tensor_valid)
            end_time1 = time()
            
            FPS = FPS + (end_time1 - start_time1)
...
if args.render:
    ...
    def run_evaluation(actions, action_filter=None):
        ...
        for action_key in actions.keys():
            ...
            gen = UnchunkedGenerator_Seq(None, poses_act, poses_2d_act, action_act,
                                     pad=pad, causal_shift=causal_shift, augment=args.test_time_augmentation,
                                     kps_left=kps_left, kps_right=kps_right, joints_left=joints_left,
                                     joints_right=joints_right)
            FPS, e1, e2, e3, ev = evaluate(gen, action_key)
