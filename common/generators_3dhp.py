...
class ChunkedGenerator_Seq:
    ...
    def __init__(self, batch_size, cameras, poses_3d, poses_2d,
                 chunk_length, stride, pad=0, causal_shift=0,
        ...
        for key in poses_2d.keys():
            ...
            # Calculate the number of chunks with the new stride
            n_chunks = (poses_2d[key].shape[0] + stride - 1) // stride
    
            # Calculate the offset for centering the chunks
            offset = (n_chunks * stride - poses_2d[key].shape[0]) // 2
    
            # Calculate the bounds with the new stride
            bounds = np.arange(n_chunks + 1) * stride - offset
    
            # Ensure bounds stay within the data limits
            bounds = np.clip(bounds, 0, poses_2d[key].shape[0] - chunk_length)
    
            # Remove duplicate bounds
            bounds = np.unique(bounds)
    
            augment_vector = np.full(len(bounds) - 1, False, dtype=bool)
            pairs += [(key, start, start + chunk_length, False) for start in bounds[:-1] if start + chunk_length <= poses_2d[key].shape[0]]
    
            if augment:
                pairs += [(key, start, start + chunk_length, True) for start in bounds[:-1] if start + chunk_length <= poses_2d[key].shape[0]]
