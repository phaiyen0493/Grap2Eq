...
def draw_3d_image_select(pred_all, gt_all, skeleton, azim, sub, act, cam, gt_2d, pred_2d):
    ...
    pred_mean = np.mean(pred_all, axis=1, keepdims=True)
    ...
    errors_2d = torch.norm(pred_2d - gt_2d, dim=len(gt_2d.shape) - 1)
    select_ind = torch.min(errors_2d, dim=1, keepdims=True).indices
    select_ind = select_ind.unsqueeze(-1).repeat(1, 1, 1, 1, 3)
    pred_all_torch = torch.from_numpy(pred_all)
    pred_select = torch.gather(pred_all_torch, 1, select_ind)
