...
class  Grap2Eq(nn.Module):
    ...
    def STE_forward(self, x, xf_proj):
        b, f, n, c = x.shape
        x = rearrange(x, 'b f n c  -> (b f) n c', )
        x = self.Spatial_patch_to_embedding(x)
        x += self.Spatial_pos_embed
        xf_proj = xf_proj.view(xf_proj.shape[0], 1, 1, xf_proj.shape[1])
        xf_proj = xf_proj.repeat(1, f, n, 1)
        xf_proj = rearrange(xf_proj, 'b f n c  -> (b f) n c', )
    ...
    def ST_w_vel_ref(self, x, vel_3D):
    ...
    def forward(self, x_2d, text, pre_text_tensor):
        b, f, n, c = x_2d.shape
        
        xf_proj, xf_out = self.encode_text(text, pre_text_tensor)

        x, vel_3D = self.STE_forward(x_2d, xf_proj)
        
        x = rearrange(x, '(b f) n cw -> (b n) f cw', f=f)
        vel_3D = rearrange(vel_3D, '(b f) n cw -> (b n) f cw', f=f)
        
        x = self.prompt_learning(x, xf_out)
...
class  Grap2Eq_Lite(nn.Module):
    ...
    def forward(self, x_2d, text, pre_text_tensor):
        ...
        x, vel_3D = self.STE_forward(x_2d)
