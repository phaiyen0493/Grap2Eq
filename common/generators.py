...
class ChunkedGenerator_Seq:
    ...
    def __init__(self, batch_size, cameras, poses_3d, poses_2d, action,
        ...
        for i in range(len(poses_2d)):
            ...
            offset = (n_chunks * stride - poses_2d[i].shape[0]) // 2
            ...
            bounds = np.clip(bounds, 0, poses_2d[i].shape[0] - chunk_length)
    
            # Remove duplicate bounds
            bounds = np.unique(bounds)
    
            augment_vector = np.full(len(bounds) - 1, False, dtype=bool)
            pairs += [(i, start, start + chunk_length, False) for start in bounds[:-1] if start + chunk_length <= poses_2d[i].shape[0]]
    
            if augment:
                pairs += [(i, start, start + chunk_length, True) for start in bounds[:-1] if start + chunk_length <= poses_2d[i].shape[0]]
