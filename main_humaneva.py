from common.arguments_humaneva import parse_args
import os
args = parse_args()
...
if args.dataset == 'h36m':
    from common.h36m_dataset_pretrain_for_humaneva import Human36mDataset
...
elif args.keypoints == 'gt' and args.h36m_pretrain == True:
    keypoints = human36m2humaneva(keypoints)
...
# connection for HumanEva
connections = {0: [11,8], 1: [0, 2,5,14], 2: [1,3], 3: [2,4], 4: [3], 5: [1,6], 6: [5,7], 7: [6], 8: [0,9], 9: [8,10], 10: [9], 11: [0,12], 12: [11,13], 13: [12], 14: [1] }
...
if not args.evaluate:
    ...
    while epoch < args.epochs:
        ...
        if epoch % args.checkpoint_frequency == 0 and epoch >= args.save_emin:
...
def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False, newmodel=None):
    ...
    log_path = os.path.join(args.checkpoint, 'humaneva_test_log_H%d_K%d.txt' %(args.num_proposals, args.sampling_timesteps))
    f = open(log_path, mode='a')
    if action is None:
        print('----------')
    else:
